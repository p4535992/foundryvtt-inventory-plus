		...
		const ret = this.each(function () {
			...
			this.sumo = {
				...
				createElems() {
					...
					O.caption = $("<span>");
					O.CaptionCont = $(
						`<p class="CaptionCont SelectBox ${O.E.attr("class")}" ><label><i></i></label></p>`
					)
				...
				Search() {
					...
					O.ftxt = $('<input type="text" class="search-txt" value="" autocomplete="off">').on(
						"click",
						(e) => {
							e.stopPropagation();
						}
				...
				toggDis(c, i) {
					...
					if (!O.mob)
						O.optDiv.find("ul.options li.opt").eq(i).toggleClass("disabled", c).removeClass("selected");
					O.setText();
				},
					...
					if (O.is_multi) {
						...
						if (c) {
							O.E.find("option")
								.toArray()
								.filter(
									(option) => !option.selected && !option.disabled && option.style.display !== "none"
								)
								.forEach((option) => {
